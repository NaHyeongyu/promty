...
const documents: Record<LegalDocument, { title: string; eyebrow: string; sections: Array<{ title: string; body: ReactNode }> }> = {
  privacy: {
    ...
    sections: [
      { title: "Who we are", body: <p>Promty provides project memory and collaboration tools for AI-assisted software work. Questions, access or correction requests, and privacy complaints can be sent to <a href="mailto:support@promty.org">support@promty.org</a> or through <a href="/app?view=support">Support</a>.</p> },
      ...
      { title: "AI processing", body: <><p>External AI generation is optional and requested separately when you first create Project Memory. The confirmation and review screens identify the configured provider—OpenAI or Google Gemini—and let you inspect the prompts and sessions before generation. You can permanently delete collected prompt or session activity from that review before continuing.</p><p>Provider requests may include bounded prompt previews and related AI response samples; project and repository metadata; changed-file paths, status and change counts; commit metadata; existing Project Memory; and generated intermediate summaries. Raw source-file contents and patches are not sent by Project Memory generation. Disabling external AI keeps ordinary workspace features available but blocks external AI generation. OpenAI API requests are sent with storage disabled where supported. Provider security, abuse-monitoring and legal retention may still apply.</p></> },
  ...
  security: {
    ...
    sections: [
      ...
      { title: "Account or content issues", body: <p>For compromised accounts, privacy requests, abuse or community content reports, use <a href="/app?view=support">Support</a>. Do not use public issue trackers for vulnerabilities.</p> },
    ],
  },
};
